...
class AuthRepository:
    ...
    def query_auth_user(self, username: str, email: str) -> Optional[Auth]:
        ...
        auth_user = session.query(UserDTO).filter((UserDTO.username == username) & (UserDTO.email ==email)).first()
        if auth_user:
            return Auth(auth_user.id, auth_user.username, auth_user.email, auth_user.hash_pw)
        else:
            return None
